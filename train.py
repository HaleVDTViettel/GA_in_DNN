...
def get_cifar100_mlp():
    """Retrieve the CIFAR dataset and process the data."""
    # Set defaults.
    nb_classes  = 100 #dataset dependent 
    batch_size  = 512
    epochs      = 10
    input_shape = (3072,) #because it's RGB
...
def compile_model_cnn(genome, nb_classes, input_shape):
    ...
    nb_layers  = genome.geneparam['nb_layers' ]
    nb_neurons = genome.nb_neurons()
    regulizer  = genome.geneparam['regulizer' ]
    activation = genome.geneparam['activation']
    optimizer  = genome.geneparam['optimizer' ]
    ...
    for i in range(0,nb_layers):
        # Need input shape for first layer.
        if i == 0:
            model.add(Conv2D(nb_neurons[i], kernel_size = (3, 3), activation = activation, padding='same', input_shape = input_shape, kernel_regularizer=l2(regulizer)))
            model.add(BatchNormalization())
        else:
            model.add(Conv2D(nb_neurons[i], kernel_size = (3, 3), activation = activation, kernel_regularizer=l2(regulizer)))
            model.add(BatchNormalization())
...
def train_and_score(genome, dataset):
    ...
    if dataset   == 'cifar10_mlp':
        ...
        nb_classes, batch_size, input_shape, x_train, x_test, y_train, y_test, epochs = get_cifar100_mlp()
    elif dataset == 'cifar100_cnn':
        nb_classes, batch_size, input_shape, x_train, x_test, y_train, y_test, epochs = get_cifar100_cnn()
